...
const reportId = searchParams.get('reportId') // You must pass ?reportId=abc in URL

useEffect(() => {
  const fetchBeforeImage = async () => {
    const res = await fetch(`/api/report?id=${reportId}`) // Adjust endpoint as needed
    const data = await res.json()
    setBeforeImageUrl(data.imageUrl) // Update this based on your DB response
  }

  if (reportId) fetchBeforeImage()
}, [reportId])
  ...
  setVerificationStatus('verifying')
    ...
    try {
    const beforeBlob = await fetch(beforeImageUrl).then(r => r.blob())
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded-xl shadow-md mb-8">
        ...
        //new
        {preview && beforeImageUrl && (
            <div className="flex gap-6 mb-6">
              <div>
                <p className="text-sm text-gray-600 mb-2">Before (Reported)</p>
                <img src={beforeImageUrl} alt="Before cleanup" className="rounded-lg max-w-xs" />
              </div>
              <div>
                <p className="text-sm text-gray-600 mb-2">After (Cleaned)</p>
                <img src={preview} alt="After cleanup" className="rounded-lg max-w-xs" />
              </div>
            </div>
          )}
